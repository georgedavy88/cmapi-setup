...
def parse_options():
    ...
    api_config_options = {'ssh_root_password': None, 'ssh_root_user': 'root', 'ssh_private_key': None,
    ...
    api_config_options.update({'kerberos': {'kdc_host': None, 'security_realm': None,
                                            'kdc_user': None, 'kdc_password': None}})

    def api_args(option, opt_str, value, *args, **kwargs):
        if option.dest == 'host_names':
            ...
            for host in value.split(','):
                ...
                api_config_options[option.dest] = [socket.gethostbyname(x.strip()) for x in value.split(',')]
        elif option.dest == 'cm_server':
            print "switch %s value check: %s" % (opt_str, value.strip())
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            api_config_options[option.dest] = socket.gethostbyname(value.strip()) if \
            ...
            api_version = get_cm_api_version(api_config_options[option.dest],
                                             api_config_options['username'],
                                             api_config_options['password'])
            print "CM API version: %s" % api_version
            api_config_options.update({'api_version': api_version})

            # from CM 5.4+ API v10 we specify 'latest' CDH version with {latest_supported}
            if int(api_config_options['api_version'].strip("v")) >= 10:
                api_config_options.update({'cdh_version': '5'})
            else:
                api_config_options.update({'cdh_version': 'latest'})

        elif option.dest == 'ssh_private_key':
            with open(value, 'r') as f:
                api_config_options[option.dest] = f.read()
        elif option.dest == 'cdh_version':
            print "switch %s value check: %s" % (opt_str, value)
            _cdh_repo = urllib2.urlopen("%s/cdh5/parcels/" % api_config_options["archive_url"]).read()
            ...
            api_config_options[option.dest] = value
    ...
    parser.add_option('-i', '--cdh-version', dest='cdh_version', type="string", action='callback',
                      callback=api_args, default='latest', help='Install CDH version. Default "latest"')
    parser.add_option('-k', '--ssh-private-key', dest='ssh_private_key', type="string", action='callback',
                      callback=api_args, help='The private key to authenticate with the hosts. '
                                              'Specify either this or a password.')
    parser.add_option('-l', '--license-file', dest='license_file', type="string", action='callback',
                      callback=api_args, help='Cloudera Manager License file name')
    parser.add_option('-m', '--cm-server', dest='cm_server', type="string", action='callback', callback=api_args,
    ...
    parser.add_option('-n', '--cluster-name', dest='cluster_name', type="string", action='callback',
                      callback=api_args, default='Cluster 1',
    ...
    parser.add_option('-p', '--ssh-root-password', dest='ssh_root_password', type="string", action='callback',
                      callback=api_args, help='*Set target node(s) ssh password..')
    parser.add_option('-u', '--ssh-root-user', dest='ssh_root_user', type="string", action='callback',
                      callback=api_args, default='root', help='Set target node(s) ssh username. Default root')
    ...
    if api_config_options['cm_server'] is None:
        parser.error(msg_req_args + "-m/--cm-server")
    else:
        if not (api_config_options['ssh_private_key'] or api_config_options['ssh_root_password']):
            parser.error(msg_req_args + "-p/--ssh-root-password or -k/--ssh-private-key")
        elif api_config_options['host_names'] is None:
            parser.error(msg_req_args + "-w/--host-names")
        elif api_config_options['ssh_private_key'] and api_config_options['ssh_root_password']:
    ...
    if not (bool(manager.get_mgmt_password("ACTIVITYMONITOR"))
        ...
        api_config_options['amon_password'] = bool(manager.get_mgmt_password("ACTIVITYMONITOR"))
        api_config_options['rman_password'] = bool(manager.get_mgmt_password("REPORTSMANAGER"))
    else:
        api_config_options['amon_password'] = manager.get_mgmt_password("ACTIVITYMONITOR")
        api_config_options['rman_password'] = manager.get_mgmt_password("REPORTSMANAGER")

    api = type('', (), api_config_options)
    check = ActiveCommands()
    cdh = ServiceActions
    if api_config_options['cm_server'] and options.teardown:
    ...
    # Uncomment here to see api configuration options
    # print api_config_options
    return options
