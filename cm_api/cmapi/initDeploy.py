...
def init_cluster():
    ...
    repo_url = ["%s/cdh5/parcels/%s" % (api.archive_url, api.cdh_version)]
    print "CDH5 Parcel URL: %s" % repo_url[0]
    api.parcel.append(manifest_to_dict(repo_url[0] + "/manifest.json"))

    # Install GPLEXTRAS5 to match CDH5 version
    repo_url.append('%s/gplextras5/parcels/%s' %
                    (api.archive_url, api.parcel[0]['version'].split('-')[0]))
    print "GPL Extras parcel URL: %s" % repo_url[1]
    api.parcel.append(manifest_to_dict(repo_url[1] + "/manifest.json"))
    ...
    if api.cluster_name in [x.name for x in api.get_all_clusters()]:
        print "Cluster name: '%s' already exists" % api.cluster_name
    else:
        print "Creating cluster name '%s'" % api.cluster_name
        api.create_cluster(name=api.cluster_name, version=api.cluster_version)
...
def add_hosts_to_cluster():
    ...
    host_list = list(set([socket.getfqdn(x) for x in api.host_names] + [socket.getfqdn(api.cm_server)]) -
                     set([x.hostname for x in api.get_all_hosts()]))
    if host_list:
        cmd = cm.host_install(user_name=api.ssh_root_user, host_names=host_list,
                              password=api.ssh_root_password, private_key=api.ssh_private_key, unlimited_jce=True)

        # TODO: Temporary fix to flag for unlimited strength JCE policy files installation (If unset, defaults to false)
        # host_install_args = {"userName": api.ssh_root_user, "hostNames": host_list, "password": api.ssh_root_password,
        #                     "privateKey": api.ssh_private_key, "unlimitedJCE": True}
        # cmd = cm._cmd('hostInstall', data=host_install_args)
        print "Installing host(s) to cluster '%s' - [ http://%s:7180/cmf/command/%s/details ]" % \
              (socket.getfqdn(api.cm_server), api.cm_server, cmd.id)
    ...
    if hosts:
        print "Adding hostId(s) to '%s'" % api.cluster_name
        print "%s" % hosts
